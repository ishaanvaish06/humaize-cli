...
A CLI for rewriting AI-generated text into more natural, human-styled prose — built to be measurably better than shallow, mainstream rewriting/paraphrasing tools, and evaluated honestly against leading AI-detection services rather than marketing claims.
...
1. **Measure, don't claim.** No fixed "95% human" number ships anywhere — not in the CLI, not in this README, not in marketing copy — without a detection-service name, service version, and a benchmark date attached. See [`plan.md` §0](./plan.md#0-scope--success-definition) and [§7.6](./plan.md#76-result-integrity-safeguards-no-fabricated-or-stale-numbers).
...
- Optionally reports diagnostics: fidelity score, burstiness stats, per-service pass/fail estimate, model checkpoint version, and benchmark date
...
Every claim this project makes about being "better" is backed by a versioned, reproducible benchmark:

- A fixed, held-out test set (`B1`) of AI-generated text across domains
- Outputs from this tool and from established mainstream rewriting tools, scored on the *same* set
- Scored on three axes, always reported together — a high evasion score achieved by damaging the text is not counted as a win:
  1. **Detection-service pass rate** — measured against a set of independent, widely used AI-detection services, reported individually per service, not blended into one number
  2. **Semantic fidelity** — NLI entailment / BERTScore vs. the original
  3. **Human-rated naturalness** — blind panel scoring
- Results carry provenance (checkpoint hash, detection-service version, benchmark date) wherever they're shown, and a report older than the refresh cadence fails to publish rather than shipping stale
...
### A note on detection-service reliability
Some AI-detection services are known to have high false-positive rates on genuinely human-written text. This project reports results against every service in its benchmark set for completeness, but treats wins against any one unreliable service as a weak signal rather than a headline number — see `plan.md` for details on how each service is weighted.

---

## Non-goals

- **Permanent, universal detector evasion.** Detection services are adversarially retrained against tools like this one. This project commits to a refresh cadence, not a one-time fix — see [`plan.md` §9](./plan.md#9-milestones).
...
- [ ] **v0.5** — First fine-tuned model (LoRA) + fidelity reranker, benchmarked against established rewriting tools
